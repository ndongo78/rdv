...
const ConfirmOrders = (props: Props) => {
  return (
    <View className="flex-1 mb-24">
      ...
      <View className="flex-1">
        {orders.map(order => (
          <TouchableOpacity 
          onPress={()=>router.push('detailCommande')}
          key={order.id} className="bg-[#1a1a1a] rounded-xl border border-slate-500 shadow-2xl  w-full  mb-4">
  ...
  />
              ...
              <View>
              <Text className="text-white font-medium">{order.date}</Text>
               <Text className="text-right text-white font-bold text-lg">{order.amount.toFixed(2)}€</Text>
              </View>
            </View>
